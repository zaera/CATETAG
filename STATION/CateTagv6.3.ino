...
Serial.println("###################");  
...
else if (incomingByte == 84)
   {
    ...
    Serial.println(" Please enter time in format HHMMSSDDMMYYYY ");
    Serial.println(" Where HH-hour MM-month SS-second DD-day MM-month YYYY-year ");
    Serial.println(" Example 13263925032015 is 13:26:39 25.03.2015 ");
...
//Battery flag - 170(AA)= 6
           ...
           if (buffer[3]==207){eeprom=1; EEPROM.write(0, eeprom);  resetFunc();}
   else if (buffer[3]==204){eeprom=2; EEPROM.write(0, eeprom);     resetFunc();}
   else if (buffer[3]==255){eeprom=3; EEPROM.write(0, eeprom);     resetFunc();}
   else if (buffer[3]==238){eeprom=4; EEPROM.write(0, eeprom);     resetFunc();}  
