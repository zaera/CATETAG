...
//Erase flag - 238(EE)  = 4
//CP flag - 207(CF)     = 1
        if (buffer[3]==207){eeprom=1; EEPROM.write(0, eeprom); cp();Serial.println("we will be CP now. CP number is: "); Serial.print(flag);  mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); return;}
   else if (buffer[3]==204){eeprom=2; EEPROM.write(0, eeprom); s(); Serial.println("we will be START now");  mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); return;}
   else if (buffer[3]==255){eeprom=3; EEPROM.write(0, eeprom); f(); Serial.println("we will be FINISH now"); mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); return;}
   else if (buffer[3]==238){eeprom=4; EEPROM.write(0, eeprom); e(); Serial.println("we will be ERASE now");  mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); return;}  
   else if (buffer[3]==221){eeprom=1; EEPROM.write(0, eeprom); cp(); Serial.println("TEST card made CP now: "); flag=flag+1; Serial.print(flag);   mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); return;}
...
Serial.println("Time sec now:");
Serial.println(tm.Second);



Serial.println("Millisec dropped down to zero 0");
}
